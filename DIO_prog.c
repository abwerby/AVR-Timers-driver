...
u8 DIO_GetPinValue(u8 u8PortIdCopy, u8 u8PinIdCopy)
{
	...
	if ((u8PortIdCopy <= PORTD) && (u8PinIdCopy <= PIN7))
	{
		...
		switch (u8PortIdCopy)
		{
			case PORTA: u8ResultLocal= GET_BIT(PINA_Reg,u8PinIdCopy); break;
			case PORTB: u8ResultLocal= GET_BIT(PINB_Reg,u8PinIdCopy); break;
			case PORTC: u8ResultLocal= GET_BIT(PINC_Reg,u8PinIdCopy); break;
			case PORTD: u8ResultLocal= GET_BIT(PIND_Reg,u8PinIdCopy); break;
		}
	}
...
void DIO_SetPortDirection (u8 u8PortId, u8 u8PortDir)
{
	...
	switch (u8PortId)
	{
		case     PORTA: DDRA_Reg = u8PortDir; break;
		case     PORTB: DDRB_Reg = u8PortDir; break;
		case     PORTC: DDRC_Reg = u8PortDir; break;
		case     PORTD: DDRD_Reg = u8PortDir; break;
